...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const priceId = plan === 'pro' 
      ? context.cloudflare.env.STRIPE_PRO_PRICE_ID
      : context.cloudflare.env.STRIPE_ENTERPRISE_PRICE_ID;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'subscription',
      success_url: `${context.cloudflare.env.APP_URL}/dashboard?success=true&plan=${plan}`,
